...
const DuplicateResumeModal = dynamic(() => import("@/components/resume/duplicate-resume-modal"), {
  ssr: false,
  loading: () => null
})
const JobResumeModal = dynamic(() => import("@/components/resume/job-resume-modal"), {
  ssr: false,
  loading: () => null
})
const ResumeBuilder = dynamic(() => import("@/app/components/resume-builder"), {
  ssr: false,
  loading: () => (
...
const TemplatePreview = dynamic(
  () => import("@/app/components/templates/preview").then(mod => ({ default: mod.TemplatePreview })),
...
const AddResumeCard = memo(({ onClick }: { onClick: () => void }) => (
  <motion.div
    ...
    onClick={onClick}
  >
    <Card className="border-2 border-dashed border-muted-foreground/20 bg-muted/10 hover:border-primary hover:bg-primary/5 hover:shadow-[0_0_30px_-5px_hsl(var(--primary)/0.2)] transition-all duration-300 h-[420px] flex items-center justify-center rounded-2xl cursor-pointer">
  ...
  ({
    ...
    return (
      <motion.div
        ...
        className="group"
      >
        <Card className="border border-border bg-card hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 rounded-2xl overflow-hidden h-[420px] flex flex-col">
          ...
          <CardContent className="space-y-4 pt-0 flex-1 flex flex-col">
            {/* TEMPLATE PREVIEW */}
            <div className="relative w-full h-48 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800/50 group-hover:bg-slate-200/70 dark:group-hover:bg-slate-800/70 transition-colors">
              {template && resume.data ? (
                /* Paper sits slightly inset with a shadow to look like a lifted document */
                <div className="absolute inset-2 rounded-lg overflow-hidden shadow-[0_3px_14px_rgba(0,0,0,0.13)] ring-1 ring-black/[0.06]">
                  <div
                  ...
                  {/* Fade to hint that the resume continues below */}
...
const LoadingSkeleton = memo(() => (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    {Array.from({ length: 8 }).map((_, i) => (
      <div key={i} className="border border-border rounded-2xl p-6 animate-pulse h-[420px] flex flex-col">
...
export default function Dashboard() {
  ...
  const [showJobResumeModal, setShowJobResumeModal] = useState(false)
  ...
  const handleJobResumeSuccess = useCallback(
    async (resumeData: any, resumeName: string, creditsRemaining: number) => {
      setShowJobResumeModal(false)
      if (!user) return

      try {
        const { data: created, error: createError } = await ResumeService.createResume({
          user_id: user.id,
          name: resumeName,
          template_id: "modern",
          data: resumeData,
        ...
        if (createError || !created) {
          toast({ title: "Error", description: "Failed to save the generated resume.", variant: "destructive" })
          return
        }

        toast({
          title: "Resume generated!",
          description: `"${resumeName}" is ready. ${creditsRemaining} credits remaining.`,
        })
    ...
    <ProtectedRoute>
      <TooltipProvider>
        <div className="min-h-screen bg-background">
          ...
          <main className="container mx-auto px-4 sm:px-6 py-8 pt-20">
            ...
            >
              ...
              <div className="flex flex-wrap justify-center gap-4">
                ...
                <Button id="job-resume-hero-btn" onClick={() => setShowJobResumeModal(true)} size="lg" variant="outline" className="px-8 py-3 border-primary/40 hover:border-primary hover:bg-primary/5">
          ...
          {showJobResumeModal && user && (
            <JobResumeModal
              isOpen={showJobResumeModal}
              onClose={() => setShowJobResumeModal(false)}
              savedResumes={savedResumes}
              userId={user.id}
              onSuccess={handleJobResumeSuccess}
            />
          )}
          {showDuplicateModal && (
